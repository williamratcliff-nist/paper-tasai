#!/usr/bin/env python3
"""Generate Figure 4 from precomputed benchmark summary JSON."""
...
DATA = ROOT / "data" / "final" / "benchmark_summary_fair_analytic_20260218.json"
OUT = ROOT / "figure4_benchmark.png"
...
METHOD_LABELS = {
    "grid": "Grid",
    "random": "Random",
    "log_gp": "Log-GP (agnostic)",
    "tasai": "TAS-AI (physics-informed)",
}
COLORS = {
    "grid": "#1f77b4",
    "random": "#ff7f0e",
...
def plot_speedup(ax, meas):
    ...
    methods = [m for m in METHODS if m != "grid"]
    width = 0.22
    for idx, method in enumerate(methods):
    ...
    ax.set_title("Relative efficiency (higher is faster)")
...
def main():
    ...
    plot_bars(
        ...
        "Final error after 300 measurements",
    )
    ...
    fig.suptitle("Analytic benchmark summary (threshold 0.20, max 300 measurements)")
